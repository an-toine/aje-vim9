...
control "vim-binary-links-1.0" do
    ...
    for tool in ["vimdiff", "view", "rvim", "rview", "ex", "evim", "eview"]
        describe file("/opt/aje/aje-vim90/root/usr/bin/#{tool}") do
            its("type") { should eq :symlink }
            its("link_path") { should eq "/opt/aje/aje-vim90/root/usr/bin/vim" }
        end
    end

    describe file("/opt/aje/aje-vim90/root/usr/bin/vim") do
...
control "gvim-binary-links-1.0" do
    ...
    for tool in ["gvimdiff", "gview", "rgvim", "rgview"]
        describe file("/opt/aje/aje-vim90/root/usr/bin/#{tool}") do
            its("type") { should eq :symlink }
            its("link_path") { should eq "/opt/aje/aje-vim90/root/usr/bin/gvim" }
        end
    end

    describe file("/opt/aje/aje-vim90/root/usr/bin/gvim") do
        its("type") { should eq :file }
        its("mode") { should cmp "0755" }
    end
end
...
control "gvim-is-working-1.0" do
    ...
    describe bash("scl enable aje-vim90 'gvim --version'") do
        its("exit_status") { should eq 0 }
    end
