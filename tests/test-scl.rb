control "scl-runtime-package-is-installed-1.0" do
    ...
    describe package("aje-vim90") do
        it { should be_installed }
    end

    describe package("aje-vim90-runtime") do
        it { should be_installed }
    end
end
...
control "scl-dev-packages-are-installed-1.0" do
    ...
    describe package("aje-vim90-build") do
        it { should be_installed }
    end

    describe package("aje-vim90-scldevel") do
        it { should be_installed }
    end
end
...
control "scl-can-be-enabled-1.0" do
    ...
    describe file("/opt/aje/aje-vim90/enable") do
        its("mode") { should cmp "0755" }
        its("content") { should match %r{export PATH=".*"} }
    end

    describe bash("scl enable aje-vim90 'echo $PATH'") do
        its("stdout") { should match %r{/opt/aje/aje-vim90/root/usr/bin:/opt/aje/aje-vim90/root/usr/sbin:.*} }
